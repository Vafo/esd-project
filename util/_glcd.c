...
uint8_t   xchar, ychar;				   /* x character(0-7), y character(0-19) */

uint8_t font[95][5] = {				   /* 5x7 ASCII character font draw a uint8_t downwards */
	...
	void cmndl( uint8_t cmd )			// left 128x64
	{	
	...
	void cmndr( uint8_t cmd )			// right 128x64
	{
	...
	void cmnda( uint8_t cmd )			// both 128x64
	{
		PORTA = cmd ;				// DATA pin PORTA			
		ClrBit(PORTE,PORTE4);		// RS
		SetBit(PORTE,PORTE6);	
	...
	/* 1 character output  */	
	void datal( uint8_t dat )          // left 128x64
	{
	...
	void datar( uint8_t dat )          // right 128x64
	{
		PORTA = dat ;				// DATA pin PORTA	
		SetBit(PORTE,PORTE4);		// PORTE.4 = 1;  // RS
		SetBit(PORTE,PORTE6);
	...
	void dataa( uint8_t dat )          // both 128x64
	{
		PORTA = dat ;				// DATA pin PORTA
		SetBit(PORTE,PORTE4);		// PORTE.4 = 1;  // RS
		SetBit(PORTE,PORTE6);
	...
	void  lcd_char(uint8_t character)
	{
		...
		if(ychar <=9)
			datal(0x00);							/* last uint8_t 0x00 making 6x8 pixel per a character */
		else
			datar(0x00);							/* last uint8_t 0x00 making 6x8 pixel per a character*/
	}


	/* character sequence output */
	void  lcd_string(uint8_t x,uint8_t y,char *string)
	{
		xchar = x;
		ychar = y;
		lcd_xy(x,y);
	...
	void GLCD_Axis_xy(unsigned char x, unsigned char y)	// draw a uint8_t downwards
	{
